...
SELECT TOP 8 
    j.nombreSede AS ubicacion,  -- Ahora mostramos la Sede raíz
    COUNT(DISTINCT ee.idEstacion) AS total
...
-- Reemplazamos el join de ubicacion por la CTE de jerarquia
INNER JOIN jerarquia j ON amb.idUbicacion = j.idUbicacion
GROUP BY 
    j.nombreSede
ORDER BY 
    total DESC;
...
SELECT TOP 15
    ...
    j.nombreSede    AS ubicacion, -- Ahora muestra la Sede (Raíz)
    COUNT(ee.idActivo) AS total
FROM inventario.estacionActivo ee
INNER JOIN inventario.estacion est  ON ee.idEstacion  = est.idEstacion
INNER JOIN inventario.asignacion a  ON est.idEstacion = a.idEstacion
    AND a.fechaLiberacion IS NULL
INNER JOIN inventario.ambiente amb  ON a.idAmbiente   = amb.idAmbiente
-- Reemplazamos la tabla ubicacion por el CTE jerarquia
INNER JOIN jerarquia j ON amb.idUbicacion = j.idUbicacion
GROUP BY 
    amb.descripcion, 
    j.nombreSede
...
$jerarquia = queryRows($conn, "
    ...
    LEFT JOIN inventario.estacionActivo ee ON est.idEstacion = ee.idEstacion
...
SELECT TOP 5
    ...
    a.fechaAsignacion,
    amb.descripcion AS ambiente,
    j.nombreSede AS sede -- Obtenemos la Sede Raíz
...
LEFT JOIN jerarquia j ON amb.idUbicacion = j.idUbicacion -- Unimos con la jerarquía
      ...
      <div class="col-12 col-md-6 col-lg-3">
        <a href="?module=inventario&action=tipoActivos" style="text-decoration:none">
          <div class="card shadow-sm card-modern h-100"
            style="border-radius:16px;transition:all .2s ease;border:1px solid var(--tblr-border-color)">
            <div class="card-body">
              <div class="d-flex justify-content-between align-items-start mb-3">
                <div
                  style="width:50px;height:50px;border-radius:12px;display:flex;align-items:center;justify-content:center"
                  class="bg-primary-lt text-primary">
              ...
              <div class="text-muted small mb-3">Tipo Activos, tipos, características, ubicaciones e IPs del sistema.
              </div>
              <hr class="my-2">
        ...
        <a href="?module=inventario&action=activos" style="text-decoration:none">
          <div class="card shadow-sm card-modern h-100"
            ...
            <div class="card-body">
              ...
              <div class="text-muted small mb-3">Administración y control de los activos registrados en inventario.
              </div>
              <hr class="my-2">
              <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="card-body">
              <div class="d-flex justify-content-between align-items-start mb-3">
                <div
                  ...
                  <i class="ti ti-device-desktop" style="font-size:24px"></i>
                </div>
        ...
        <a href="?module=inventario&action=asignaciones" style="text-decoration:none">
          <div class="card shadow-sm card-modern h-100"
            ...
            <div class="card-body">
              ...
              <div class="d-flex justify-content-between align-items-center">
                <div>
                  <span class="badge bg-success-lt text-success" style="font-size:.72rem"><?= $pctAsignadas ?>%
                    asignadas</span>
    ...
    <div class="row g-3 mb-4">
      ...
      <div class="col-12 col-lg-3">
        <div class="dash-card h-100">
          ...
          <div class="dash-card-body d-flex flex-column align-items-center justify-content-center gap-3"
            ...
            <?php else: ?>
              ...
              <div class="timeline-item">
                ...
                <div class="flex-grow-1">
                  ...
                  <!-- Contenedor de Etiquetas (Sede, Ambiente, Fecha) -->
                  <div class="d-flex gap-2 mt-1 flex-wrap">
                    
                    <!-- Nueva etiqueta de Sede -->
                    <?php if (!empty($a['sede'])): ?>
                    <span class="badge badge-outline text-secondary" style="font-size:.65rem">
                      <i class="ti ti-building me-1"></i><?= htmlspecialchars($a['sede']) ?>
                    </span>
                    <?php endif; ?>

                    <!-- Etiqueta de Ambiente -->
                    <?php if (!empty($a['ambiente'])): ?>
                    <span class="badge bg-primary-lt text-primary" style="font-size:.65rem">
                      <?= htmlspecialchars($a['ambiente']) ?>
                    </span>
                    <?php endif; ?>

                    <!-- Etiqueta de Fecha -->
                    <span class="badge badge-outline text-muted" style="font-size:.65rem">
                      <?= fmtFecha($a['fechaAsignacion']) ?>
                    </span>
