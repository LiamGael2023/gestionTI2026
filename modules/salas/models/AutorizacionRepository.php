 ...
 * Operaciones de aprobación, rechazo e historial de reservas (rol Autorizador/Admin).
 *
 * Principio SOLID aplicado: SRP — solo gestiona el flujo de autorización.
...
class AutorizacionRepository extends BaseModel
{
    ...
    public function aprobarReserva(int $id_reserva, int $id_autorizador): array
    {
        ...
        if ($reserva['estado'] !== self::ESTADO_PENDIENTE)
            return ['ok' => false, 'msg' => 'La reserva no está en estado PENDIENTE.'];
    ...
    /**
     * Rechaza una reserva PENDIENTE con observación opcional.
     */
    public function rechazarReserva(int $id_reserva, int $id_autorizador, string $observacion = ''): array
    {
        ...
        // Obtener datos antes de rechazar para la notificación
        $info = $this->fetchOne(
            "SELECT r.motivo,
                    CONVERT(varchar(10), r.fecha, 120) AS fecha,
                    CONVERT(varchar(5), r.hora_inicio, 108) AS hora_inicio,
    ...
    /**
     * Cancela automáticamente las reservas PENDIENTE cuya fecha+hora_inicio ya pasó.
     * Se llama en cada petición AJAX del módulo (sin necesidad de cron job).
     *
    ...
    public function cancelarReservasVencidas(): int
    {
        ...
        foreach ($vencidas as $r) {
            MailService::notificarReservaCancelada(
                ...
                'La reserva fue cancelada automáticamente porque el horario ya expiró sin ser aprobada.'
            );
        }
    ...
    /**
     * Historial de cambios de estado de una reserva específica (timeline).
     */
