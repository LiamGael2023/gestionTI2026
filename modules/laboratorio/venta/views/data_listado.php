...
while ($row = sqlsrv_fetch_array($stmtData, SQLSRV_FETCH_ASSOC)) {
    ...
    if ($row['Activo'] == 1) {
        $acciones = '<div class="btn-group btn-group-sm" role="group">' .
                    ...
                    ($puedeEliminar ? '<button type="button" class="btn btn-ghost-danger" onclick="eliminarVenta(' . $row['Id_Producto'] . ')" title="Eliminar"><i class="ti ti-trash"></i></button>' : '') .
                    '</div>';
    } else {
        $acciones = '<div class="btn-group btn-group-sm" role="group">' .
                    ($puedeEditar ? '<button type="button" class="btn btn-ghost-success" onclick="reactivarVenta(' . $row['Id_Producto'] . ')" title="Reactivar"><i class="ti ti-check"></i></button>' : '<span class="text-muted small">Inactivo</span>') .
                    '</div>';
    }
