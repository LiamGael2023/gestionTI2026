...
try {
    ...
    $total = $model->contarPorEstado('Finalizado', $tipoServicio);
    $totalFiltered = ($search !== '') ? $model->contarPorEstado('Finalizado', $tipoServicio, $search) : $total;
    $muestras = $model->obtenerPorEstado('Finalizado', $start, $length, $tipoServicio, $search);
    ...
    foreach ($muestras as $row) {
        ...
        $fechaFirma = valorTexto($row['Fecha_Validacion'] ?? null);
        $estado = valorTexto($row['Estado'] ?? null);
        $tipo = valorTexto($row['TipoMuestra'] ?? null);
        $agricultorUrl = rawurlencode($agricultor);
        $accion = '';
        $accion .= '<a class="btn btn-sm btn-primary me-1" href="?module=laboratorio&action=muestra&subaction=resultados_pasados&id_muestra=' . $id . '&id_cliente=' . $id_cliente . '&agricultor=' . $agricultorUrl . '" title="Ver resultados"><i class="ti ti-eye"></i></a>';
        $accion .= '<a class="btn btn-sm btn-success" href="modules/laboratorio/muestra/controllers/ExportarResultadosPasados.php?id_muestra=' . $id . '" title="Descargar Excel"><i class="ti ti-file-spreadsheet"></i></a>';

        $data[] = [$id, $agricultor, $valle, $servicio, $fechaAnalisis, $fechaFirma, $estado, $tipo, $accion];
    }
