...
if (empty($placa)) {
    ...
    $datos_vehiculo = ControladorVehiculo::ctrMostrarVehiculoReporteHistorialPapeleta($placa);

    if (!$datos_vehiculo || count($datos_vehiculo) === 0) {
        $pdf->SetFont('helvetica', '', 12);
        $pdf->Cell(0, 10, 'No se encontró información para la placa ingresada.', 0, 1, 'C');
    } else {
        $pdf->Cell(0, 10, 'PLACA DEL VEHÍCULO: ' . safeText($datos_vehiculo[0]['placa'] ?? ''), 1, 1, 'L');
        $marca = safeText($datos_vehiculo[0]['marca'] ?? '');
        ...
        foreach ($datos_vehiculo as $row) {
            ...
            $conductor = safeText($row['conductor'] ?? '');
            $gerencia = safeText($row['gerencia'] ?? '');
            $lugar = safeText($row['lugar'] ?? '');

            $html .= '<tr>
                <td align="center">' . safeText($row['papeleta']) . '</td>
                <td>' . $conductor . '</td>
                <td>' . $gerencia . '</td>
                <td>' . $lugar . '</td>
